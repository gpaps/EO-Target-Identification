...
from _register_Ship_dataset import register_datasets
...
class ClassAwareMapper(DatasetMapper):
    def __init__(self, cfg, is_train=True):
        ...
        self.extra_aug = [
            ...
            T.RandomFlip(prob=2.0, horizontal=True, vertical=False)  # Always flip
            # T.RandomApply(T.RandomRotation(angle=[-2, 2]), prob=0.3),
            # T.RandomApply(T.RandomBrightness(0.95, 1.2), prob=0.3),
            # T.RandomApply(T.RandomContrast(0.95, 1.2), prob=0.3),
...
def setup_and_train(output_dir, num_classes, lr=0.0001, roi_batch=512, nms=0.5, score=0.5):#  backbone="r50"):
    cfg = get_cfg()
    # Backbone selection
    # Swin config file from the modified Detectron2 fork (adjust path if needed)
    cfg.merge_from_file("configs/SwinT/swin_tiny_patch4_window7_224.yaml")

    # Path to Swin pretrained weights
    cfg.MODEL.WEIGHTS = "./pretrained/swin_tiny_patch4_window7_224.pth"
    cfg.MODEL.BACKBONE.NAME = "build_swin_transformer_fpn_backbone"
...
#
    ...
    # cfg.MODEL.WEIGHTS = "/home/gpaps/esa-train/Ships_/Optical/trained_models_Ship_Optical/run_1/Optical_sweep_lr0
    # .00015_b512_s0.5_nms0.4/model_final.pth"
    ...
    try:
        log_val_predictions_to_tensorboard(
            cfg,
            trainer.model,
            val_loader,
            os.path.join(output_dir, "tensorboard"),
            max_images=10,
            global_step=cfg.SOLVER.MAX_ITER
        )
        # log_val_predictions_to_tensorboard(self.trainer.cfg, self.model, self.val_loader,
        #                                    os.path.join(self.output_dir, "tensorboard"),
        #                                    max_images=10,
...
if __name__ == "__main__":
    ...
    output_dir = f"./trained_models_Ship_Optical/TEST0000/Optical_sweep_{args.name}"
