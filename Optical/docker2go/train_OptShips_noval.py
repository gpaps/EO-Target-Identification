...
from utils.augmentations import GaussianBlurAll
from samplers.balanced_sampler import BalancedSampler
from rel_register_Ship_dataset import register_datasets
...
class ClassAwareMapper(DatasetMapper):
    def __init__(self, cfg, is_train=True):
        ...
        # Standard augmentations (apply to all)
        self.standard_aug = [
            T.ResizeShortestEdge(short_edge_length=(558, 698), sample_style="range"),
        ...
        self.extra_aug = [
            T.RandomExtent(scale_range=(0.8, 1.0), shift_range=(0.05, 0.05)),  # more aggressive zoom
            # T.RandomCrop("relative_range", (0.5, 0.6)),  # smaller partial object crop
            T.RandomApply(T.RandomRotation(angle=[-5, 5]), prob=0.15),
            # T.RandomContrast(0.9, 1.1),
            T.RandomLighting(0.1),
            # GaussianBlurAll(sigma_range=(1.1, 1.2), prob=0.2),
        ]

        # Target classes (from cfg.RARE_CLASS_IDS)
        self.target_class_ids = getattr(cfg, "RARE_CLASS_IDS", [2, 4])
    ...
    def __call__(self, dataset_dict):
        ...
        # Apply standard + extra aug only to rare-class images
        ...
        try:
            ...
            if len(annos) == 0:
                ...
                print(f"⚠ Unhandled error in {dataset_dict['file_name']}: {e}")
                return None
    ...
    def after_step(self):
        ...
        if next_iter % self._period == 0 or is_final:
            ...
            if self.trainer.cfg.TEST.ENABLE_TB_IMAGES:
                ...
                #                                    )
                log_confusion_and_f1(csv_path, self.class_names, os.path.join(self.output_dir, "tensorboard"),
                                     next_iter)
...
def log_val_predictions_to_tensorboard(cfg, model, data_loader, tb_dir, max_images=8, global_step=0):
    ...
    for idx, inputs in enumerate(data_loader):
        ...
        # writer.add_image(f"val_prediction_{idx}", out.get_image(), 0, dataformats="HWC")
        # writer.add_image(f"val_prediction_{idx}", out.get_image(), self.trainer.iter, dataformats="HWC")
        # writer.add_image(f"val_prediction_{idx}", out.get_image(), cfg.SOLVER.MAX_ITER, dataformats="HWC")
...
def setup_and_train(output_dir, num_classes, lr=0.0001, roi_batch=512, nms=0.5, score=0.5, backbone="r50", max_iter=40000):
    cfg = get_cfg()
    ...
    cfg.SOLVER.WARMUP_ITERS = int(0.025 * max_iter)
    ...
    cfg.DATASETS.TEST = ("Ship_Optical_val",)  # will be ignored in sweep but necessary for hooks
    cfg.DATALOADER.NUM_WORKERS = 16
    cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS = False

    # Input size
    cfg.INPUT.MIN_SIZE_TRAIN = (558, 698)
    cfg.INPUT.MAX_SIZE_TRAIN = 1047
    ...
    cfg.MODEL.ROI_HEADS.POSITIVE_FRACTION = 0.75  # Optional, keep or remove
    ...
    cfg.KEEP_GT_IN_VAL = True  # For posthoc eval
    cfg.TEST.ENABLE_TB_IMAGES = True
    cfg.TEST.ENABLE_AIR_EVAL = True
    cfg.TEST.EVAL_PERIOD = 999999  # Effectively disables mid-training eval

    # Final prep
    os.makedirs(output_dir, exist_ok=True)
    logText(cfg, cfg.OUTPUT_DIR)

    # Trainer setup and run
    ...
    # Post-training metrics and visuals
    ...
    if os.path.exists(final_pred_csv):
        try:
            ...
            print(f"Could not evaluate prediction log: {e}")
    ...
    pd.DataFrame([summary_row]).to_csv(summary_csv, mode="a", header=not os.path.exists(summary_csv), index=False)
...
if __name__ == "__main__":
    ...
    output_dir = f"./trained_models_Ship_Optical/run_noval2/Optical_sweep_{args.name}"
