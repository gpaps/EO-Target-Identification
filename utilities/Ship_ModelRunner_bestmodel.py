...
if __name__ == "__main__":
    if SWEEP_MODE:
        ...
        for model_folder in model_dirs:
            ...
            # Try to extract best_iter from metrics.json
                ...
                with open(metrics_path, "r") as f:
                    for line in reversed(f.readlines()):
                        try:
                            ...
                            best_iter = record.get("best_iter", None)
                            if best_iter:
                                candidate = f"model_{best_iter:07d}.pth"
            ...
            # Fallback to latest model_*.pth
            if not best_pth:
                pth_files = [f for f in os.listdir(folder_path) if f.startswith("model_") and f.endswith(".pth")]
                if not pth_files:
                    print(f"⚠️ No .pth models in {folder_path}")
                    continue
                best_pth = max(pth_files, key=lambda f: os.path.getmtime(os.path.join(folder_path, f)))

            model_path = os.path.join(folder_path, best_pth)
            ...
            output_dir = os.path.join("../../Inference_result/Optical/singleRun_run_1/bestmodel_/", model_folder)
            # output_dir = os.path.join("./Inference_result/Optical/run_noval2/bestmodel_/", model_folder)
            # output_dir = os.path.join("./Inference_result/Optical/SkySAT_boostAP_/bestmodel_/", model_folder)
            os.makedirs(output_dir, exist_ok=True)
            # store the used model name
            with open(os.path.join(output_dir, "used_model.txt"), "w") as f:
                f.write(f"{best_pth}\n")

            print(f"✅ Running inference on: {model_folder} → {best_pth}")
